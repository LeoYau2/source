...
lowpass_filter = np.zeros((crop[2],crop[3],3), dtype=np.float32)
for y in range(crop[2]):
    for x in range(crop[3]):
        if (y-crop[2]/2)*(y-crop[2]/2)+(x-crop[3]/2)*(x-crop[3]/2) > 10000:
            lowpass_filter[y,x,:] = 0
        else:
            lowpass_filter[y,x,:] = 1
print(f"generate_lowpass_filter:{time.perf_counter()-start_time_section:.5f}s")
cv2.imshow('lowpass_filter',lowpass_filter)
...
start_time_section = time.perf_counter()
fft_result = fft2(crop_img)
print(f"fft:{time.perf_counter()-start_time_section:.5f}s")

start_time_section = time.perf_counter()
fft_result_shifted = fftshift(fft_result) # 進行平移操作，將低頻成分移到中央
print(f"fftshift:{time.perf_counter()-start_time_section:.5f}s")
print(np.min(fft_result_shifted), np.max(fft_result_shifted))
filtered_fft_result = fft_result_shifted * lowpass_filter
print(np.min(filtered_fft_result), np.max(filtered_fft_result))
print(fft_result.shape)
print(fft_result_shifted.shape)
fft_result_real = np.real(fft_result)
fft_result_shifted_real = np.real(fft_result_shifted)
fft_result_abs = np.abs(fft_result)
fft_result_shifted_abs = np.abs(fft_result_shifted)
print(np.min(fft_result_real), np.max(fft_result_real))
print(np.min(fft_result_shifted_real), np.max(fft_result_shifted_real))
print(np.min(fft_result_abs), np.max(fft_result_abs))
print(np.min(fft_result_shifted_abs), np.max(fft_result_shifted_abs))

fft_result_real = (fft_result_real+800000)/2000000
fft_result_shifted_real = (fft_result_shifted_real+800000)/2000000
fft_result_abs = fft_result_abs/1060000
fft_result_shifted_abs = fft_result_shifted_abs/1060000

cv2.imshow('fft_result_real',fft_result_real)
cv2.imshow('fft_result_shifted_real',fft_result_shifted_real)
cv2.imshow('fft_result_abs',fft_result_abs)
cv2.imshow('fft_result_shifted_abs',fft_result_shifted_abs)

print(fft_result_real[100,100])
print(fft_result_real[200,200])


start_time_section = time.perf_counter()
ifft_result = ifft2(ifftshift(filtered_fft_result))
print(f"inverse Fourier transform:{time.perf_counter()-start_time_section:.5f}s")

#restored_image = np.zeros((crop[2],crop[3],3), dtype=np.uint8)
restored_image = np.real(ifft_result)
temp_int32 = (np.clip(np.clip(restored_image, 0, 65535/gain) * gain, 0, 65535)).astype(np.int32)
restored_sRGB_8bpc = (np.clip(linear_to_sRGB[temp_int32]*255, 0, 255)).astype(np.uint8)
print(restored_image[100,100])
print(restored_image[100,200])


cv2.imshow('restored_image',restored_image)
cv2.imshow('restored_sRGB_8bpc',restored_sRGB_8bpc)

#linear_sRGB_bloom_img = linear_sRGB_bloom_img[bloom_range:bloom_range+crop[2], bloom_range:bloom_range+crop[3], :]
temp_int32 = (np.clip(np.clip(crop_img, 0, 65535/gain) * gain, 0, 65535)).astype(np.int32)
