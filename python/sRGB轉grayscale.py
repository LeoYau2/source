...
print(f"generate_sRGB_to_linear_lookup_table_8bit:{time.time()-start_time_section:.3f}s")

start_time_section = time.time()
generate_linear_to_sRGB_lookup_table_16bit() #製作linear轉sRGB查找表
print(f"generate_linear_to_sRGB_lookup_table_16bit:{time.time()-start_time_section:.3f}s")
...
print(f"sRGB_to_linear:{time.time()-start_time_section:.3f}s")

start_time_section = time.time()
for i in range(width):
    for j in range(height):
        linear_grayscale_img[j,i] = linear_sRGB_to_linear_grayscale(linear_sRGB_img[j,i,:]) #把線性sRGB轉換成線性的黑白圖片
print(f"linear_sRGB_to_linear_grayscale:{time.time()-start_time_section:.3f}s")

start_time_section = time.time()
for i in range(width):
    for j in range(height):
        grayscale_img[j,i] = int(linear_to_sRGB[int(linear_grayscale_img[j,i]*65535)]*255) #把線性黑白圖片轉換成8位元深度sRGB曲線灰階圖片，因為把float32量化成16bit離散訊號，所以會有一點誤差
print(f"linear_to_sRGB:{time.time()-start_time_section:.3f}s")
