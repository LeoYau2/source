...
#from scipy.signal import kaiser_filter
...
for i in range (1024):
   frequency_response[i] *= 10**(gaussian(i, 500, 20)*20*(2*math.pi)**0.5*(-12)/20)
...
fir_coeffs *= gain*32767

plt.figure(figsize=(16, 5))
plt.plot(fir_coeffs)
plt.show()
write("C:\Program Files\EqualizerAPO\config\EqualizerAPO\outfilter.2.wav", 192000, fir_coeffs.astype(np.int16))
