...
def sRGB_EOTF_vectorized(V_sRGB): #把sRGB訊號轉換為線性訊號，輸入和輸出範圍都是0~1，使用numpy進行向量化計算
...
print(f"generate_sRGB_to_linear_lookup_table_8bit:{time.time()-start_time_section:.5f}s")
...
print(f"generate_linear_to_sRGB_lookup_table_16bit:{time.time()-start_time_section:.5f}s")

start_time_section = time.time()
linear_sRGB_img = sRGB_to_linear[input_img] #把線性sRGB訊號轉換為線性黑白訊號，使用NumPy向量化查表，速度很快
print(f"sRGB_to_linear:{time.time()-start_time_section:.5f}s")

start_time_section = time.time()
linear_grayscale_img = linear_sRGB_img[:,:,0]*0.299+linear_sRGB_img[:,:,1]*0.587+linear_sRGB_img[:,:,2]*0.114 #把線性sRGB轉換成線性的黑白圖片，使用numpy向量乘以純量，速度很快

'''
if __name__ == '__main__':
    manager = Manager()
    linear_grayscale_img = manager.Array('f', height * width) # 创建可共享数组
    
    pool = Pool(processes=4) # 创建4个进程的进程池
    for y in range(height):
        pool.apply_async(process_row, (y,)) # 异步提交每个行的任务
    pool.close()
    pool.join()
    linear_grayscale_img = np.frombuffer(linear_grayscale_img.get_obj()).reshape(height, width)
    '''
#print(linear_grayscale_img[64,64])
print(f"linear_sRGB_to_linear_grayscale:{time.time()-start_time_section:.5f}s")
...
for y in range(height):
    for x in range(width):
        temp[y,x] = int(linear_grayscale_img[y,x]*65535) #把float32量化成16bit離散訊號，所以會有一點誤差'''

'''threads = []
for y in range(height):
    t = threading.Thread(target=process_row, args=(y,)) 
    threads.append(t)
    t.start()
...
'''if __name__ == '__main__':
    temp = process_image(linear_grayscale_img)'''

temp_int32 = np.zeros((height,width), dtype=np.int32) #建立暫存index的陣列
temp_int32 = (linear_grayscale_img * 65535).astype(np.int32) #把float32量化成16bit離散訊號，所以會有一點誤差，使用numpy向量乘以純量和類型轉換，速度很快
...
print(f"linear_to_sRGB:{time.time()-start_time_section:.5f}s")
