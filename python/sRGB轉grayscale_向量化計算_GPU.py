...
use_gpu = np.array([0,0,1,1,0], dtype=np.uint8)
...
if use_gpu[3]==1:
    linear_sRGB_img = cp.asarray(linear_sRGB_img)
...
linear_grayscale_img = cp.sum(linear_sRGB_img[:, :, :3] * cp.array([0.299, 0.587, 0.114]), axis=2)
print(f"linear_sRGB_to_linear_grayscale:{time.perf_counter()-start_time_section:.5f}s")

start_time_section = time.perf_counter()

if (use_gpu[4] == 0) and (use_gpu[3] == 1):
    linear_grayscale_img = linear_grayscale_img.get()

#linear_grayscale_img_np = np.asarray(linear_grayscale_img)
#linear_grayscale_img_np = linear_grayscale_img.get()

print(f"cupy to numpy:{time.perf_counter()-start_time_section:.5f}s")
...
def sRGB_OETF_vectorized_gpu(L): #把線性訊號轉換為sRGB訊號，輸入和輸出範圍都是0~1
    V_sRGB = cp.where(L > 0.0031308, (1 + 0.055) * L ** (1/2.4) - 0.055, 12.92 * L)
    return V_sRGB
...
def generate_linear_to_sRGB_lookup_table_16bit_vectorized_gpu(): #製作一個linear轉sRGB查找表，輸入範圍是0~65535,輸出範圍是0~1，這個查找表可以保留16EV動態範圍
    linear_values = cp.arange(0, 65536, dtype=cp.float32) / 65535.0
    linear_to_sRGB_vectorized = sRGB_OETF_vectorized_gpu(linear_values)
    return linear_to_sRGB_vectorized

use_gpu = np.array([0,0,1,1,1], dtype=np.uint8)
...
linear_sRGB_img = np.zeros((height,width,3), dtype=np.float32) #用來儲存線性sRGB彩色圖片，範圍0~1
linear_grayscale_img = cp.zeros((height,width), dtype=cp.float32) #用來儲存線性灰階圖片，範圍0~1
grayscale_img = np.zeros((height,width), dtype=np.uint8) #用來儲存8位元深度sRGB曲線灰階圖片，範圍0~255
sRGB_to_linear = np.zeros((256), dtype=np.float32) #用來儲存8bit查找表，需要256*4bytes=1kb記憶體
linear_to_sRGB = np.zeros((65536), dtype=np.float32) #用來儲存16bit查找表，需要65535*4bytes=256kb記憶體
...
if use_gpu[1]==0:
    ...
    for i in range(1):
        linear_to_sRGB = generate_linear_to_sRGB_lookup_table_16bit_vectorized_gpu()
...
for i in range(1):
    linear_grayscale_img = cp.sum(linear_sRGB_img[:, :, :3] * cp.array([0.299, 0.587, 0.114]), axis=2)
#linear_grayscale_img = cp.dot(linear_sRGB_img[:, :, :3], [0.299, 0.587, 0.114])
print(f"linear_sRGB_to_linear_grayscale:{time.perf_counter()-start_time_section:.5f}s")
...
if use_gpu[1]==0:
    ...
    temp_int32 = cp.zeros((height,width), dtype=cp.int32) #建立暫存index的陣列
    temp_int32 = (linear_grayscale_img * 65535).astype(cp.int32) #把float32量化成16bit離散訊號，所以會有一點誤差，使用numpy向量-純量乘法和類型轉換，速度很快
grayscale_img = linear_to_sRGB[temp_int32] #把16bit線性黑白圖片轉換成8位元深度sRGB曲線灰階圖片，使用NumPy向量化查表，速度很快
temp_int32 = None #把temp_int32設為None，讓temp_int32成為垃圾回收的候選對象
#gc.collect() #手動垃圾回收，不是必要的，釋放temp_int32之前占用的記憶體
print(f"linear_to_sRGB:{time.perf_counter()-start_time_section:.5f}s")
...
grayscale_img_numpy = cp.asnumpy(grayscale_img)

#cv2.imshow('input_img',input_img) #顯示輸入圖片
#cv2.imshow('linear_sRGB_img',linear_sRGB_img) #顯示線性sRGB圖片
#cv2.imshow('linear_grayscale_img',linear_grayscale_img) #顯示線性黑白圖片
cv2.imshow('grayscale_img',grayscale_img_numpy) #顯示8位元深度sRGB曲線灰階圖片'''
