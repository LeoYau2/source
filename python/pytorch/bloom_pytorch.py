...
#tifffile.imwrite(f"sRGB_bloom_+{brightness_EV}EV.range{bloom_range}.fp32.tiff", rgb_img, compression=tifffile.COMPRESSION.ADOBE_DEFLATE)
#tifffile.imwrite(f"sRGB_bloom_+{brightness_EV}EV.range{bloom_range}.fp32.tiff", rgb_img, compression='LZW')

if use_half_precision_exr == 1:
    rgb_img = rgb_img.astype('float16')
    
uint16_img = (rgb_img.astype('float16')).view(np.uint16)
# 將最終結果儲存為 EXR 檔案
if use_crop == 0:
...
if use_crop == 0:
    if use_bloom_process == 1:
        ...
        cv2.imwrite(f"sRGB_bloom_+{brightness_EV}EV.range{bloom_range}.uint16.png", uint16_img)
        
    elif use_bloom_process == 0:
        cv2.imwrite(f"sRGB_+{brightness_EV}EV.png", sRGB_8bpc)
        cv2.imwrite(f"sRGB_+{brightness_EV}EV.uint16.png", uint16_img)
elif use_crop == 1:
    if use_bloom_process == 1:
        cv2.imwrite(f"crop_sRGB_bloom_+{brightness_EV}EV.range{bloom_range}.png", sRGB_8bpc)
        cv2.imwrite(f"crop_sRGB_bloom_+{brightness_EV}EV.uint16.png", uint16_img)
    elif use_bloom_process == 0:
        cv2.imwrite(f"crop_sRGB_+{brightness_EV}EV.png", sRGB_8bpc)
        cv2.imwrite(f"crop_sRGB_+{brightness_EV}EV.uint16.png", uint16_img)
