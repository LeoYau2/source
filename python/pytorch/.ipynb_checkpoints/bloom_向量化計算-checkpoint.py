...
kernel = np.zeros((1, 3, bloom_range*2+1, bloom_range*2+1))
...
for y in range(bloom_range*2+1):
    for x in range(bloom_range*2+1):
        if (x!=bloom_range) or (y!=bloom_range):
            kernel[0,:,x,y] = ((y-bloom_range)*(y-bloom_range)*bloom_size*bloom_size+(x-bloom_range)*(x-bloom_range)*bloom_size*bloom_size)**-1
print(weight[0][0])
print(np.sum(weight))
