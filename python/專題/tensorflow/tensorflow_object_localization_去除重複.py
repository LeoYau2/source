    ...
    for i in range(boxes.shape[1]):
        ...
        if np.any(scores[0,i] > score_threshold): #score大於score_threshold
            ...
            for j in range(num_objects+1):
                #print(ymin, xmin, ymax, xmax, "i=",i)
                #print("result",result_array[j,1]/img.shape[0],result_array[j,0]/img.shape[1],result_array[j,3]/img.shape[0],result_array[j,2]/img.shape[1],"j=",j)
...
num_objects = detect(img, 0.3, 0.05) #接收方框數量，輸入圖片 和 最低score倍率 和 最低score 和 判斷不同方框太接近的最小差異，最低score越高偵測出數量越少
