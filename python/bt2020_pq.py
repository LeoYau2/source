...
# 设置 x264 的 qp 值
output_stream.options = {'preset':'2','crf': '16'}
#output_stream.options = {'cpu-used':'8','crf': '16'}
#start_frame_index = 1000
rgb_frames = []
yuv_frames = []
yuv_frame = np.zeros((int(height*1.5),width), dtype=np.uint8)
for frame in container.decode(video=0):
    ...
    yuv_frame = (rgb_frame * 4).astype(np.uint8)
    converted_frame = av.VideoFrame.from_ndarray(yuv_frame, format='yuv420p')
...
yuv_concat = np.concatenate(yuv_frame, axis=0) 

(
  ffmpeg
  .input('pipe:', format='rawvideo', pix_fmt='yuv420p', s='1920x1080') 
  .output('output.mp4')
  .run(pipe_stdin=True, pipe_stdout=False, input_data=yuv_concat.tobytes())  
)
