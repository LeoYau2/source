...
foreach($file in Get-ChildItem -Filter *.$input_container)
{
    ...
    $ffmpeg_arguments = "-hide_banner -y -i `"$file`" `"$dir\$na.png`""
    $scriptBlock = {
        $process_ = Start-Process -FilePath "C:\Users\Leo\ffmpeg.exe" -ArgumentList $ffmpeg_arguments -PassThru -WindowStyle Hidden
        $process_.PriorityClass = "Idle"
        Wait-Process -InputObject $process_
        #$process_.WaitForExit()
        #Start-Sleep -Milliseconds 4000
        #ffmpeg -hide_banner -y -i "$file" "$dir\$na.png" | Out-Null
        #Start-Sleep -Milliseconds 4000
