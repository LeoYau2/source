...
function estimate_qp {
    ...
    $y = [double]$ln_size + [math]::Log([double]$target_size / [double]$size)
...
pause
$encode = process_files -files $list -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp_array $list_qp -alphaQ $alphaQ -speed $test_speed -maxConcurrency $maxConcurrency
#pause
...
while ($temp.Count -gt 0){
    ...
    #pause
    $encode = process_files -files $too_big -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp $too_big_qp -alphaQ $alphaQ -speed $test_speed -maxConcurrency $maxConcurrency
    #pause
}
...
while ($temp.Count -gt 0){
    ...
    Write-Output "list_qp: $list_qp"
    #pause
    $encode = process_files -files $too_small -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp $too_small_qp -alphaQ $alphaQ -speed $test_speed -maxConcurrency $maxConcurrency
    #pause
}
...
while ([math]::Abs([math]::Log([double]$avg_size / [double]$target_avg_size)) -gt [double]$max_ln_size_error){
    pause
    ...
    $encode = process_files -files $remain -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp $remain_qp -alphaQ $alphaQ -speed $test_speed -maxConcurrency $maxConcurrency
    for ($i = 0; $i -lt $remain_id.Count; $i++) {
        $file = $list[$remain_id[$i]]
        $na = [io.path]::GetFileNameWithoutExtension("$file")
        $test_file_path = "$dir\${na}.${target_width}.qp$last_qp.avif"
