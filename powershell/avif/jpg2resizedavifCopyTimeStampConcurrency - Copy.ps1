...
#-files $files -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp $qp -alphaQ $alphaQ -speed $speed -maxConcurrency $maxConcurrency
...
$runspaces = @()
$processingFiles = @{}#>
$test = test_function -text "test"

$encode = process_files -files $files -dir $dir -target_width $target_width -jobs $jobs -yuv $yuv -depth $depth -range $range -qp $qp -alphaQ $alphaQ -speed "6" -maxConcurrency $maxConcurrency
#$encode = process_files2 -dir $dir
<#foreach ($file in $files) {
    $na = [io.path]::GetFileNameWithoutExtension("$file")
    $arguments = "-j $jobs -y $yuv -d $depth -r $range --speed $speed --max $qp --min $qp --maxalpha $alphaQ --minalpha $alphaQ -s $speed `"$dir\$na.${target_width}.png`" -o `"$dir\$na.${target_width}.qp$qp.avif`""
    $ffmpeg_arguments = "-hide_banner -i `"$file`" -vf `"format=rgba,scale=${target_width}:-1`" -sws_flags area `"$dir\$na.${target_width}.png`""
    ...
    $powershell = [powershell]::Create().AddScript($scriptBlock).AddArgument($target_width).AddArgument($ffmpeg_arguments).AddArgument($arguments).AddArgument($file.FullName).AddArgument($dir).AddArgument($qp).AddArgument($priorityClass)
    $powershell.RunspacePool = $runspacePool
