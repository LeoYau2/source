...
//g++ .\hdr_brightness_gain.cpp -fopenmp -mavx -lavformat -lavcodec -lavutil -IC:/Users/Leo/Documents/program/ffmpeg/ffmpeg-6.1.1/ -IC:\Users\Leo\Documents\program\ffmpeg\ffmpeg-6.1.1\build\include -LC:\Users\Leo\Documents\program\ffmpeg\ffmpeg-6.1.1\build\lib -o hdr_brightness_gain.exe
#include <stdio.h>
#include <immintrin.h>
#include <iostream>
...
/*
    ...
    if (!videoStream) {
        fprintf(stderr, "無法創建視訊流\n");
        return -1;
    }

    AVCodecContext* codecContext = avcodec_alloc_context3(codec);
    if (!codecContext) {
        fprintf(stderr, "無法分配編碼器上下文\n");
        return -1;
    }

    videoStream->codecpar->codec_id = AV_CODEC_ID_H265;
    videoStream->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
    videoStream->codecpar->format = AV_PIX_FMT_YUV420P10;
    videoStream->codecpar->width = 960;
    videoStream->codecpar->height = 540;

    avcodec_parameters_to_context(codecContext, videoStream->codecpar);

    // 其他編碼器參數的設置，比如比特率、GOP大小等
