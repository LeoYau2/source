...
int main(int argc, char *argv[]){
    ...
    if(found_file){
        ...
        for(int i=0;i<total_frame_count;i++){
            ...
            input_file.seekg(static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index) * sizeof(unsigned short), ios::beg);
            input_file.read(reinterpret_cast<char*>(Y), y_size * sizeof(unsigned short));

            input_file.seekg((static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)) * sizeof(unsigned short), ios::beg);
            input_file.read(reinterpret_cast<char*>(U), u_size * sizeof(unsigned short));

            input_file.seekg((static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)+static_cast<streamoff>(u_size)) * sizeof(unsigned short), ios::beg);
    ...
    /*
            ...
            */
            ...
            //Y = 0.2298625*R + 0.59325*G + 0.0518875*B
            //U = -0.1396*R -0.3604*G + 0.5*B + 0.5
            //V = 0.5*R - 0.4598*G - 0.0402*B + 0.5
            ...
            /*array_scalar_multiply_40(temp_1, Y_float, 0.85546875, y_size);
            array_scalar_add_40(Y_float, temp_1, 64, y_size);
            array_scalar_multiply_40(temp_2, U_full, 0.875, y_size);
            array_scalar_add_40(U_full, temp_2, 64, y_size);
            array_scalar_multiply_40(temp_3, V_full, 0.875, y_size);
            array_scalar_add_40(V_full, temp_3, 64, y_size);*/

            float_clip_min_40(Y_float, 0, y_size);
            //cout<<"step 8" << endl;*/
    /*
            for(int i=0;i<1;i++){
