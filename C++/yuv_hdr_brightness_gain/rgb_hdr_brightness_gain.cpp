...
int main(int argc, char *argv[]){
    ...
    //int del = 1;
    bool found_file;
    int frame_index = 0, total_frame_count, yuv_size = width*height*1.5, y_size = width*height, u_size = width*height/4, v_size = width*height/4, rgb_size = width*height*3, r_size = width*height, g_size = width*height, b_size = width*height;
    ...
    if(found_file){
        ...
        cout << filesize/rgb_size/sizeof(unsigned short) << " frames" << endl;
        total_frame_count = filesize/rgb_size/sizeof(unsigned short);
        unsigned short* R = new unsigned short[y_size];
        unsigned short* G = new unsigned short[y_size];
        unsigned short* B = new unsigned short[y_size];
        unsigned short* rgb = new unsigned short[y_size*3];
        unsigned short* R_out = new unsigned short[y_size];
        unsigned short* G_out = new unsigned short[y_size];
        unsigned short* B_out = new unsigned short[y_size];
        ...
        float* temp_Y = new float[y_size];
        float* temp_U = new float[y_size];
        float* temp_V = new float[y_size];
        ...
        //    cout<<PQ_gain_table_10bit[i] << endl;
        //}
        //cout<<"step 2" << endl;
        ...
        for(int i=0;i<total_frame_count;i++){
            ...
            input_file.seekg(static_cast<streamoff>(rgb_size) * static_cast<streamoff>(frame_index) * sizeof(unsigned short), ios::beg);
            input_file.read(reinterpret_cast<char*>(rgb), rgb_size * sizeof(unsigned short));

            /*input_file.seekg((static_cast<streamoff>(rgb_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)) * sizeof(unsigned short), ios::beg);
            input_file.read(reinterpret_cast<char*>(G), g_size * sizeof(unsigned short));

            input_file.seekg((static_cast<streamoff>(rgb_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)+static_cast<streamoff>(y_size)) * sizeof(unsigned short), ios::beg);
            input_file.read(reinterpret_cast<char*>(B), b_size * sizeof(unsigned short));*/
            //cout<<yuv_size<<endl;
            //cout<<"seek:" << static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index) * sizeof(unsigned short) << ", " << (static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)) * sizeof(unsigned short) << ", " << (static_cast<streamoff>(yuv_size) * static_cast<streamoff>(frame_index)+static_cast<streamoff>(y_size)+static_cast<streamoff>(u_size)) * sizeof(unsigned short) << endl;
            //cout<<"step 3" << endl;
            ...
            frame_min = 512;
            for (size_t i = 0; i < y_size; ++i) {
                if (Y_float[i] > frame_max) {
                    frame_max = Y_float[i];
                }

                if (Y_float[i] < frame_min) {
                    frame_min = Y_float[i];
                }
            }
            cout<<"min:"<<frame_min<<", max:"<<frame_max<<endl;*/
            /*
            float_clip_min_40(Y_float, 64, y_size);

            array_scalar_add_40(temp_1, Y_float, -64, y_size);
            array_scalar_multiply_40(Y_float, temp_1, 1.167808, y_size);
            array_scalar_add_40(temp_1, U_full, -64, y_size);
            array_scalar_multiply_40(U_full, temp_1, 1.141741, y_size);
            array_scalar_add_40(temp_1, V_full, -64, y_size);
            array_scalar_multiply_40(V_full, temp_1, 1.141741, y_size);
            frame_max = 512;
            frame_min = 512;
            for (size_t i = 0; i < y_size; ++i) {
            ...
            /*array_scalar_add_40(temp_4, Y_float, -64, y_size);
            array_scalar_multiply_40(temp_1, temp_4, 1.164, y_size);
            ...
            cout<<"min:"<<frame_min<<", max:"<<frame_max<<endl;*/
            //float_clip_min_40(R, 0, y_size);
            //float_clip_min_40(G, 0, y_size);
            //float_clip_min_40(B, 0, y_size);
            //cout<<"step 6" << endl;
    /*
            for (size_t i = 0; i < y_size; i++) {
            ...
            /*for (int i = 0; i < adjustedSize; i+=8) {
                __m256i R_vec = _mm256_loadu_si128(&R[i]);
                __m256i G_vec = _mm256_loadu_si128(&G[i]);
                __m256i B_vec = _mm256_loadu_si128(&B[i]);
                //__m256i R_index = _mm256_cvtps_epi32(R_vec);
                //__m256i G_index = _mm256_cvtps_epi32(G_vec);
                //__m256i B_index = _mm256_cvtps_epi32(B_vec);
                __m256 R_gain = _mm256_i32gather_ps(PQ_gain_table_10bit, R_vec, 4);
                __m256 G_gain = _mm256_i32gather_ps(PQ_gain_table_10bit, G_vec, 4);
                __m256 B_gain = _mm256_i32gather_ps(PQ_gain_table_10bit, B_vec, 4);
                _mm256_storeu_si128(&R[i], R_gain);
                _mm256_storeu_si128(&G[i], G_gain);
                _mm256_storeu_si128(&B[i], B_gain);
            }
            ...
            for (int i = 0; i < rgb_size; ++i) {
                rgb[i] = PQ_gain_table_16bit[rgb[i]];
                //R_out[i] = PQ_gain_table_16bit[G[i]];
                //G_out[i] = PQ_gain_table_16bit[G[i]];
                //B_out[i] = PQ_gain_table_16bit[B[i]];
            }
            //cout<<"step 7" << endl;
    /*
    ...
    /*
            ...
            */
            ...
            array_scalar_multiply_40(temp_1, Y_float, 0.856305, y_size);
            array_scalar_add_40(Y_float, temp_1, 64, y_size);
            array_scalar_multiply_40(temp_2, U_full, 0.875855, y_size);
            array_scalar_add_40(U_full, temp_1, 64, y_size);
            array_scalar_multiply_40(temp_3, V_full, 0.875855, y_size);
            array_scalar_add_40(V_full, temp_3, 64, y_size);*/
            //cout<<"step 8" << endl;*/
    /*
            for(int i=0;i<1;i++){
...
/*
            ...
            output_file.write(reinterpret_cast<const char*>(rgb), rgb_size * sizeof(unsigned short));
            /*output_file.write(reinterpret_cast<const char*>(R_out), y_size * sizeof(unsigned short));
            output_file.write(reinterpret_cast<const char*>(G_out), y_size * sizeof(unsigned short));
            output_file.write(reinterpret_cast<const char*>(B_out), y_size * sizeof(unsigned short));*/
            //cout<<"step 11" << endl;
            frame_index++;
