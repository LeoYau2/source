...
for %%a in ("%inputPath%\*.mkv") do (
	ffmpeg -hide_banner -hwaccel nvdec -y -i "%inputPath%\%%~na.mkv" -pix_fmt yuv420p10le "%yuvPath%\%%~na.yuv"
	yuv_hdr_brightness_gain.multithread.exe %yuvPath%\%%~na.yuv %gainedyuvPath%\%%~na.yuv %width% %height% %gain%
